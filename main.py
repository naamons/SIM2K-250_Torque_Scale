...
if rpm_axis and torque_airmass_axis and not airmass_map.empty:
    if len(airmass_map.columns) != len(rpm_axis):
        ...
        airmass_map.index.name = 'Torque (Nm)'
else:
    st.stop()
...
if torque_map_axis and not torque_map.empty:
    if len(torque_map.columns) != len(rpm_axis):
        ...
        torque_map.index.name = 'Air Mass (MG/STK)'
else:
    st.stop()
...
1. **Copy and Paste the OE Tables**: Paste your original (OE) torque and air mass tables into the respective text areas on the left sidebar.
2. **Input New Desired Torque**: Enter your new target torque value in the "New Maximum Torque Axis Value" section.
3. **Apply Scaling**:
    - After generating the scaled maps, copy the three highlighted green rows from the **Extended Air Mass Map** and **Extended Torque Map**.
    - Paste these new rows into your existing torque and air mass tables.
4. **Update Axis Data**: Replace the old axis data with the new axis values generated by the tool.
5. **Repeat as Needed**: Ensure this process is done for all torque maps and air mass maps, including any reference maps.

**Note**: Properly scaling these maps ensures optimal ECU performance, controlling timing, boost, clutch pressure, and more.
""")
